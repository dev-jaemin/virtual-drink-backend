...
const createReceiverPeerConnection = (socketID, socket, roomID) => {
    ...
    pc.ontrack = (e) => {
        if (wsServerState.users[roomID]) {
            if (!isIncluded(wsServerState.users[roomID], socketID)) {
        ...
        let others = getOtherUsersInRoom(socketID, roomID);
...
export default (wsServer, socket) => {
    ...
    //         '\nwsServerState.senderPCs ]] ',
    //         wsServerState.senderPCs,
    //         // "\nwsServerState.receiverPCs ]] ", wsServerState.receiverPCs,
    //         '\nusers ]] ',
    //         wsServerState.users,
    //         '\nwsServerState.socketToRoom ]] ',
    //         wsServerState.socketToRoom
    //     );
    // }, 3000);
